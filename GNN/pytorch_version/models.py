...
class StudentGraphSAINT(nn.Module):
    def __init__(self, num_classes, arch_gcn, train_params, feat_full, label_full, cpu_eval=False):
        """
        Inputs:
            arch_gcn            parsed arch of GCN
            train_params        parameters for training
        """
        super(StudentGraphSAINT,self).__init__()
        self.use_cuda = (args_global.gpu >= 0)
        if cpu_eval:
            self.use_cuda=False
        if "attention" in arch_gcn:
    ...
    def _loss(self, preds, labels, norm_loss, from_teacher=None):
        if self.sigmoid_loss:
            norm_loss = norm_loss.unsqueeze(1)
            loss = torch.nn.BCEWithLogitsLoss(weight=norm_loss,reduction='sum')(preds, labels)
        else:
            _ls = torch.nn.CrossEntropyLoss(reduction='none')(preds, labels)
            loss = (norm_loss * _ls).sum()
        if from_teacher is not None:
            loss += SoftCrossEntropy(preds, from_teacher, reduction='sum')
        return loss
    ...
    def train_step(self, node_subgraph, adj_subgraph, norm_loss_subgraph, from_teacher):
        """
        Forward and backward propagation
        """
        self.train()
        self.optimizer.zero_grad()
        preds,labels,labels_converted = self(node_subgraph, adj_subgraph)    # will call the forward function
        loss = self._loss(preds,labels_converted,norm_loss_subgraph, from_teacher[node_subgraph]) # labels.squeeze()?
        loss.backward()
        torch.nn.utils.clip_grad_norm(self.parameters(), 5)
        self.optimizer.step()
        return loss,self.predict(preds),labels
